...
def get_disk_space(path):
	if sys.platform.find('linux')!=-1:
		return statevfs_unix(path)
	return statevfs(path)
...
def readImageDuration(imagefile,ffmpeg='ffmpeg'):
	import re
	rst = () # (creattime,lastmodifytime) timestamp time ticks
	imagefile = os.path.normpath(imagefile)
	cmd = u'/usr/local/bin/ffprobe  %s'%(imagefile)
