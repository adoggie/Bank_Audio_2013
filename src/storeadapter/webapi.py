...
def syncPrepare(r):
	...
	try:
		...
		rs = cm.Terminal.objects.filter(id = user['id'])
		if not rs:
			return FailCallReturn(ecode= AppErrors.UNAUTHORIZED,emsg='term not found,'
			                                                             'should relogin').httpResponse()
...
def syncFile(r):
	...
	try:
		...
		if not rs:
			return FailCallReturn(ecode= AppErrors.OBJECT_NOT_FOUND,emsg='term not found,'
			                                                             'should relogin').httpResponse()
